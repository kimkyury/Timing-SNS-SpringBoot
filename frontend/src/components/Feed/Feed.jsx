...
function Feed(data) {
  ...
  const formatT = (date) => {
    ...
    const differenceInMinutes =
      (currentTime - postTime) / 60000 - koreaTimezoneOffset; // 시간 차이(분)

    const correctedPostTime = new Date(
      postTime.getTime() + (localTimezoneOffset - koreaTimezoneOffset) * 60000
    ); // 시간을 한국 시간대로 보정
  ...
  const handleScroll = () => {
    ...
    if (scrollHeight + windowHeight > document.body.offsetHeight - 1) {
      setPage((prevPage) => {
        if (
          prevPage !=
          Math.floor(state.commentCount / 10) +
            (state.commentCount % 10 > 0 ? 1 : 0)
        ) {
